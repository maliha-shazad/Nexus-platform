...
export const MeetingCalendar: React.FC<{ userRole: string; userId: string }> = ({ userId }) => {
  ...
  useEffect(() => {
    const savedAvailability = localStorage.getItem(`availability_${userId}`);
    const savedMeetings = localStorage.getItem(`meetings_${userId}`);
  ...
  const addAvailabilitySlot = () => {
    if (selectedSlot) {
      ...
      localStorage.setItem(`availability_${userId}`, JSON.stringify(updated));
      setShowModal(false);
      setMeetingTitle('');
    }
  };
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-gray-500">Click on any time slot to add availability or schedule a meeting</p>
        </CardHeader>
        <CardBody>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <h3 className="text-md font-medium text-gray-900">Pending Meetings ({pendingMeetings.length})</h3>
          </CardHeader>
          <CardBody>
            {pendingMeetings.length === 0 ? (
              <p className="text-gray-500 text-sm">No pending meetings</p>
            ) : (
